...
/**
 * This class handles request for DiagnosticReport Resource. It generates a parameter resource based on read/search request types
...
public class DiagnosticReportResourceProvider extends AbstractJaxRsResourceProvider<DiagnosticReport> {
	 ...
	 * this method: http://<server name>/<context>/fhir/DiagnosticReport/1
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public DiagnosticReport readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,@IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with DiagnosticReport Read operation :{} at {}",theId.getIdPart(),Instant.now());
		...
		try {
			...
			pathParams.put("resourceType",ResourceTypes.DIAGNOSTICREPORT.toString());	
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.DIAGNOSTICREPORT.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.DIAGNOSTICREPORT.toString());
					if (obj instanceof OperationOutcome) {
						...
						diagnosticReport = (DiagnosticReport)obj;
					}
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of DiagnosticReportResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in DiagnosticReportResourceProvider class ");
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,		
			...
			@Description(shortDefinition = "The status of the diagnostic report") @OptionalParam(name = DiagnosticReport.SP_STATUS) TokenAndListParam theStatus,

			@Description(shortDefinition = "The resource identity")
	        @OptionalParam(name = DiagnosticReport.SP_RES_ID)
			TokenAndListParam theId,
			
			@Description(shortDefinition = "A code or name that describes this diagnostic report") @OptionalParam(name = DiagnosticReport.SP_CODE) TokenAndListParam theCode,

			@Description(shortDefinition = "This is used for searching, sorting and display purposes.") @OptionalParam(name = DiagnosticReport.SP_CATEGORY) TokenAndListParam theCategory,

			@Description(shortDefinition = "The time or time-period the observed values are related to") @OptionalParam(name = DiagnosticReport.SP_DATE) DateAndListParam theDate,

			@Description(shortDefinition = "The subject of the report if a patient") @OptionalParam(name = DiagnosticReport.SP_PATIENT) ReferenceAndListParam thePatient,
			@Description(shortDefinition = "Encounter DiagnosticReport was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,
			
			@IncludeParam(allow = {"DiagnosticReport:performer"})
	        Set<Include> theIncludes,
			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with DiagnosticReport search with parameters :{} at {}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all DiagnosticReport search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap,ResourceTypes.DIAGNOSTICREPORT.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of DiagnosticReportResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in DiagnosticReportResourceProvider class ");
		return retVal;
	}
