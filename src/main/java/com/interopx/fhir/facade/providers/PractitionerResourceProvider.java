...
/**
 * This class handles request for Practioner Resource. It generates a parameter resource based on read/search request types
...
public class PractitionerResourceProvider extends AbstractJaxRsResourceProvider<Practitioner> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Practitioner/1
	 * @param request
	 * @param response
	...
	public Resource readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails, @IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with Practitioner Read operation :{} at {} ",theId.getIdPart() ,Instant.now());
		...
		try {
			...
			pathParams.put("resourceType",ResourceTypes.PRACTITIONER.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.PRACTITIONER.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.PRACTITIONER.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of PractitionerResourceProvider class : ");
			throw e;
		}
		return practitioner;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
		...
		@Description(shortDefinition = "A Practitioner identifier") @OptionalParam(name = Practitioner.SP_IDENTIFIER) TokenAndListParam theIdentifier,
		@Description(shortDefinition = "A portion of either family or given name of the Practitioner") @OptionalParam(name = Practitioner.SP_NAME) StringAndListParam theName,
		@IncludeParam(reverse = true, allow = { "Provenance:target",
					"PractitionerRole:practitioner" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with Practitioner search with parameters :{} at{}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all Practitioner search");
		}
		...
		try {
			...
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap,
					ResourceTypes.PRACTITIONER.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of PractitionerResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in PractitionerResourceProvider class ");
		return retVal;
	}
