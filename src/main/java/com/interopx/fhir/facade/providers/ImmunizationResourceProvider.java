...
/**
 * This class handles request for Immunization Resource. It generates a
...
public class ImmunizationResourceProvider extends AbstractJaxRsResourceProvider<Immunization> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Immunization/1
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Immunization readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails, @IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with Immunization Read operation :{} at {}",theId.getIdPart() ,Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.IMMUNIZATION.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.IMMUNIZATION.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.IMMUNIZATION.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of ImmunizationResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in ImmunizationResourceProvider class ");
		return immunization;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Immunization.SP_RES_ID) TokenAndListParam theId,
			@Description(shortDefinition = "Vaccination (non)-Administration Date") @OptionalParam(name = Immunization.SP_DATE) DateAndListParam theDate,

			@Description(shortDefinition = "The patient for the vaccination record") @OptionalParam(name = Immunization.SP_PATIENT) ReferenceAndListParam thePatient,

			@Description(shortDefinition = "Encounter immunization was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,

			@Description(shortDefinition = "Immunization event status") @OptionalParam(name = Immunization.SP_STATUS) TokenAndListParam theStatus,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with Immunization search with parameters :{} at {}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all Immunization search");
		}
		...
		try {
			...
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap,
					ResourceTypes.IMMUNIZATION.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() for ImmunizationProvider class : ");
			throw e;
		}
		logger.debug("End search() in ImmunizationResourceProvider class ");
