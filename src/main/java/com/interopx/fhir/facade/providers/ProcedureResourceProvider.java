...
/**
 * This class handles request for Procedure Resource. It generates a parameter
...
public class ProcedureResourceProvider extends AbstractJaxRsResourceProvider<Procedure> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Procedure/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Procedure readOrVread(HttpServletRequest request, HttpServletResponse response,
			RequestDetails theRequestDetails, @IdParam IdType theId) throws Exception {
		logger.debug(" Request received on FHIR FACADE with Procedure Read operation :{} at {}", theId.getIdPart(),
				Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.PROCEDURE.toString());
			...
			logger.debug("{}{}{} Sending the message to OutQueue at {}", AppConstants.DEBUG_REQUEST_ID, request,
					AppConstants.DEBUG_FHIR_FACADE_EVENT, Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.PROCEDURE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.PROCEDURE.toString());
			...
			logger.error("Exception in readOrVread() of ProcedureResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in ProcedureResourceProvider class ");
		return procedure;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Procedure.SP_RES_ID) TokenAndListParam theId,
			@Description(shortDefinition = "When the procedure was performed") @OptionalParam(name = Procedure.SP_DATE) DateAndListParam theDate,

			@Description(shortDefinition = "A code to identify a  procedure") @OptionalParam(name = Procedure.SP_CODE) TokenAndListParam theCode,

			@Description(shortDefinition = "Search by subject - a patient") @OptionalParam(name = Procedure.SP_PATIENT) ReferenceAndListParam thePatient,

			@Description(shortDefinition = "Encounter Procedure was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,

			@Description(shortDefinition = "preparation | in-progress | not-done | suspended | aborted | completed | entered-in-error | unknown") @OptionalParam(name = Procedure.SP_STATUS) TokenAndListParam theStatus,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with Procedure search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all Procedure search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.PROCEDURE.toString());
			...
			logger.error("Exception in search() of ProcedureResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in ProcedureResourceProvider class");
		return retVal;
	}
