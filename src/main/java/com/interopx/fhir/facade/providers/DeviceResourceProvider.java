...
/**
 * This class handles request for DeviceResource Resource. It generates a
...
public class DeviceResourceProvider extends AbstractJaxRsResourceProvider<Device> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Device/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Device readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,@IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with Device Read operation :{} at {}",theId.getIdPart(),Instant.now());
		Device device = null;
		try {
			...
			pathParams.put("resourceType", ResourceTypes.DEVICE.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.DEVICE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource resource = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.DEVICE.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of DeviceResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in DeviceResourceProvider class ");
		return device;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Device.SP_RES_ID) TokenAndListParam theId,

			@Description(shortDefinition = "Technical endpoints providing access to services operated for the organization") @OptionalParam(name = Device.SP_TYPE) TokenAndListParam theType,

			@Description(shortDefinition = "Encounter Device was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,

			@Description(shortDefinition = "Patient information, If the device is affixed to a person") @OptionalParam(name = Device.SP_PATIENT) ReferenceAndListParam thePatient,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with Device search with parameters :{} at {}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all Device search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.DEVICE.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of DeviceResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in DeviceResourceProvider class ");
		return retVal;
	}
