...
/**
 * This class handles request for Observation Resource. It generates a parameter
...
public class ObservationResourceProvider extends AbstractJaxRsResourceProvider<Observation> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Observation/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Observation readOrVread(HttpServletRequest request, HttpServletResponse response,
			RequestDetails theRequestDetails, @IdParam IdType theId) throws Exception {
		logger.debug("Request received on FHIR FACADE with Observation Read operation :{} at {}", theId.getIdPart(),
				Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.OBSERVATION.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.OBSERVATION.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.OBSERVATION.toString());
			...
			logger.error("Exception in readOrVread() of ObservationResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in ObservationResourceProvider class ");
		return observation;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Observation.SP_RES_ID) TokenAndListParam theId,
			...
			@Description(shortDefinition = "Obtained date/time. If the obtained element is a period, a date that falls in the period") @OptionalParam(name = Observation.SP_DATE) DateAndListParam theDate,
			...
			@Description(shortDefinition = "Encounter observation was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,
			...
			@IncludeParam(allow = { "Observation:performer" }) Set<Include> theIncludes,
			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with Observation search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all Observation search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.OBSERVATION.toString());
			...
			logger.debug("{}{}{} Message sent to OutQueue at {}", AppConstants.DEBUG_REQUEST_ID, requestId,
					AppConstants.DEBUG_FHIR_FACADE_EVENT, Instant.now());
			// Saving the request to database
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.OBSERVATION.toString());
			service.saveRequest(requestResponseLog);
			CompletableFuture<RequestResponseLog> requestResponseObj = service.getResponseFromQueue(requestId);
			RequestResponseLog responseLog = requestResponseObj.get();
			if (responseLog != null) {
			...
			logger.error("Exception in search() of ObservationResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in ObservationResourceProvider class");
		return retVal;
	}
