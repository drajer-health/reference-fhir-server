...
/**
 * This class handles request for CarePlan Resource. It generates a parameter
...
public class CarePlanResourceProvider extends AbstractJaxRsResourceProvider<CarePlan> {
	 ...
	 */
	@Read
	public CarePlan readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails, @IdParam IdType theId)
			throws Exception {
logger.debug(" Request received on FHIR FACADE with CarePlan Read operation : {} at {} ",theId.getIdPart() ,Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.CAREPLAN.toString());
...
logger.debug("{}{}{} Sending the message to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.CAREPLAN.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource resource = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.CAREPLAN.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of CarePlanResourceProvider class: ",e);
			throw e;
		}
		logger.debug("End readOrVread() in CarePlanResourceProvider class ");
		return carePlan;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "Time period plan covers") @OptionalParam(name = CarePlan.SP_DATE) DateAndListParam theDate,
			...
			@Description(shortDefinition = "Encounter care plan was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,
		...
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					" Request received on FHIR FACADE with CarePlan search with parameters : {} at {}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all CarePlan search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.CAREPLAN.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of CarePlanResourceProvider class: ");
			throw e;
		}
		logger.debug("End search() in CarePlanResourceProvider class ");
		return retVal;
	}
