...
/**
 * This class handles request for MedicationRequest Resource. It generates a
...
public class MedicationRequestResourceProvider extends AbstractJaxRsResourceProvider<MedicationRequest> {
	 ...
	 * this method: http://<server name>/<context>/fhir/MedicationRequest/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public MedicationRequest readOrVread(HttpServletRequest request, HttpServletResponse response,
			RequestDetails theRequestDetails, @IdParam IdType theId) throws Exception {
		logger.debug("Request received on FHIR FACADE with MedicationRequest Read operation :{} at {}",
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.MEDICATIONREQUEST.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.MEDICATIONREQUEST.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.MEDICATIONREQUEST.toString());
					if (obj instanceof OperationOutcome) {
						...
						medicationRequest = (MedicationRequest) obj;
					}
				} else if (responseLog.getStatus().equals(Status.ABORTED.toString())) {
			...
			logger.error("Exception in readOrVread() of MedicationRequestResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in MedicationRequestResourceProvider class ");
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "Status of the prescription") @OptionalParam(name = MedicationRequest.SP_STATUS) TokenAndListParam theStatus,

			@Description(shortDefinition = "Returns prescriptions with different intents") @OptionalParam(name = MedicationRequest.SP_INTENT) TokenAndListParam theIntent,

			@Description(shortDefinition = "The resource identity") @OptionalParam(name = MedicationRequest.SP_RES_ID) TokenAndListParam theId,
			@Description(shortDefinition = "Return prescriptions with this encounter or episode of care identifier") @OptionalParam(name = MedicationRequest.SP_ENCOUNTER) ReferenceAndListParam theContext,

			@Description(shortDefinition = "Return prescriptions written on this date") @OptionalParam(name = MedicationRequest.SP_AUTHOREDON) DateAndListParam theAuthoredOn,

			@Description(shortDefinition = "Returns prescriptions for a specific patient") @OptionalParam(name = MedicationRequest.SP_PATIENT) ReferenceAndListParam thePatient,
		...
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with MedicationRequest search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all MedicationRequest search");
		}
		...
		try {
			...
			logger.error("Exception in search() of MedicationRequestResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in MedicationRequestResourceProvider class ");
		return retVal;
	}
