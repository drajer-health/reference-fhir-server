...
/**
 * This class handles request for Organization Resource. It generates a
...
public class OrganizationResourceProvider extends AbstractJaxRsResourceProvider<Organization> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Organization/1
	 * 
	 * @param request
	 * @param response
	...
	public Resource readOrVread(HttpServletRequest request, HttpServletResponse response,
		...
		logger.debug("Request received on FHIR FACADE with Organization Read operation :{} at {}", theId.getIdPart(),
				Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.ORGANIZATION.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.ORGANIZATION.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.ORGANIZATION.toString());
			...
			logger.error("Exception in readOrVread() of OrganizationResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in OrganizationResourceProvider class ");
		return organization;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Organization.SP_RES_ID) TokenAndListParam theId,
		...
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with Organization search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all Organization search");
		}
		...
		try {
			...
			logger.debug("{}{}{} Sending the response at {}",
					AppConstants.DEBUG_REQUEST_ID + requestId + AppConstants.DEBUG_FHIR_FACADE_EVENT,
					Instant.now());
		} catch (Exception e) {
			logger.error("Exception in search() of OrganizationResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in OrganizationResourceProvider class ");
		return retVal;
	}
