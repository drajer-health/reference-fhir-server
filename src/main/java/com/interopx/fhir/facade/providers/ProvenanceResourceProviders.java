...
/**
 * This class handles request for Provenance Resource. It generates a
...
public class ProvenanceResourceProviders extends AbstractJaxRsResourceProvider<Provenance> {
	 ...
	 * Example URL to invoke this method: http://<server name>/<context>/fhir/Provenance/1
	 * @param request
	 * @param response
	 ...
	 */
	@Read
    public Provenance readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
    										@IdParam IdType theId) throws Exception {
logger.debug("Request received on FHIR FACADE with Provenance Read operation :{} at {} ",theId.getIdPart() ,Instant.now());
		...
		try {
			...
			pathParams.put("resourceType",ResourceTypes.PROVENANCE.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.PROVENANCE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.PROVENANCE.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of ProvenanceResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in ProvenanceResourceProvider class");
		return provenance;
    }
    ...
    public Bundle search(
    	...
    	if(StringUtils.isNotBlank(request.getQueryString())) {
    		logger.debug(
					"Request received on FHIR FACADE with Provenance search with parameters :{} at {}" , request.getQueryString() ,Instant.now());   	}
    	else {
        	logger.debug("Request received for all Provenance search");
    	}
        ...
        try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.PROVENANCE.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of ProvenanceResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in ProvenanceResourceProvider class");
        return retVal;
    }
}