...
/**
 * This class handles request for PractionerRole Resource. It generates a
...
public class PractitionerRoleResourceProvider extends AbstractJaxRsResourceProvider<PractitionerRole> {
	private static final Logger logger = LoggerFactory.getLogger(PractitionerRoleResourceProvider.class);
	@Autowired
	 ...
	 * this method: http://<server name>/<context>/fhir/PractitionerRole/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Resource readOrVread(HttpServletRequest request, HttpServletResponse response,
			RequestDetails theRequestDetails, @IdParam IdType theId) throws Exception {
		logger.debug("Request received on FHIR FACADE with PractitionerRole Read operation :{} at {}",
				theId.getIdPart(), Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.PRACTITIONERROLE.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.PRACTITIONERROLE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.PRACTITIONERROLE.toString());
					if (obj instanceof OperationOutcome) {
						...
						practitionerRole = (PractitionerRole) obj;
					}
			...
			logger.error("Exception in readOrVread() of PractitionerRoleResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in PractitionerRoleResourceProvider class ");
	 ...
	 * @param theId
	 * @param theSpecialty
	 * @param thePractitionerReferenceAndListParam
	 * @param theIncludes
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = PractitionerRole.SP_RES_ID) TokenAndListParam theId,

			@Description(shortDefinition = "The practitioner has this specialty at an organization") @OptionalParam(name = PractitionerRole.SP_SPECIALTY) TokenAndListParam theSpecialty,

			@Description(shortDefinition = "Practitioner that is able to provide the defined services for the organization") @OptionalParam(name = PractitionerRole.SP_PRACTITIONER, chainWhitelist = {
					"*" }) ReferenceAndListParam thePractitionerReferenceAndListParam,

			@IncludeParam(allow = { "PractitionerRole:practitioner",
					"PractitionerRole:organization" }) Set<Include> theIncludes,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with PractitionerRole search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all PractitionerRole search");
		}
		StringAndListParam thePractitionerNameList = new StringAndListParam();
		TokenAndListParam thePractitionerIdentifierList = new TokenAndListParam();
		try {
			if (thePractitionerReferenceAndListParam != null) {
				List<ReferenceOrListParam> thePractitionerOrList = thePractitionerReferenceAndListParam
						.getValuesAsQueryTokens();
				for (ReferenceOrListParam thePractitionerParam : thePractitionerOrList) {
					List<ReferenceParam> thePractitionerReferenceParam = thePractitionerParam.getValuesAsQueryTokens();
					for (ReferenceParam refrenceParam : thePractitionerReferenceParam) {
						String chainParam = refrenceParam.getChain();
						if (chainParam != null) {
							if (Practitioner.SP_NAME.equals(chainParam)) {
								StringParam strParam = refrenceParam.toStringParam(fhirContext);
								if (Practitioner.SP_NAME.equals(chainParam)) {
									thePractitionerNameList.addAnd(strParam);
								}
							} else if (Practitioner.SP_IDENTIFIER.equals(chainParam)) {
								TokenParam tokenType = refrenceParam.toTokenParam(fhirContext);
								thePractitionerIdentifierList.addAnd(tokenType);
							}
						}
					}
				}
			...
			logger.error("Exception while getting values from chained parameters", e);
		}
		SearchParameterMap paramMap = new SearchParameterMap();
		paramMap.add(PractitionerRole.SP_RES_ID, theId);
		paramMap.add(PractitionerRole.SP_SPECIALTY, theSpecialty);
		paramMap.add(AppConstants.PRACTITIONER_NAME, thePractitionerNameList);
		paramMap.add(AppConstants.PRACTITIONER_IDENTIFIER, thePractitionerIdentifierList);
		...
		try {
			...
			logger.error("Exception in search() of PractitionerRoleResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in PractitionerRoleResourceProvider class");
		return retVal;
	}
