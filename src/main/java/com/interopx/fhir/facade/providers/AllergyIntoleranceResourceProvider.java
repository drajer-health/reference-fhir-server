...
/**
 * This class handles request for AllergyIntolerance Resource. It generates a
...
public class AllergyIntoleranceResourceProvider extends AbstractJaxRsResourceProvider<AllergyIntolerance> {
	...
	public AllergyIntolerance readOrVread(HttpServletRequest request, HttpServletResponse response,
		...
		logger.debug("Request received on FHIR FACADE with AllergyIntolerance Read operation : {} at {}",
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.ALLERGYINTOLERANCE.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.ALLERGYINTOLERANCE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource resource = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.ALLERGYINTOLERANCE.toString());
					if (resource instanceof OperationOutcome) {
						...
						allergyIntolerance = (AllergyIntolerance) resource;
					}
			...
			logger.error("Exception in readOrVread() of AllergyIntoleranceResourceProvider class:");
			throw e;
		}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = AllergyIntolerance.SP_RES_ID) TokenAndListParam theId,

			@Description(shortDefinition = "active | inactive | resolved") @OptionalParam(name = AllergyIntolerance.SP_CLINICAL_STATUS) TokenAndListParam theClinicalStatus,
			...
			@Description(shortDefinition = "Encounter when the allergy or intolerance was asserted") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with AllergyIntolerance search with parameters : {} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all AllergyIntolerance search");
		}
		...
		try {
			...
			logger.error("Exception in @Search of AllergyIntoleranceResourceProvider: ");
			throw e;
		}
		logger.debug("End search() in AllergyIntoleranceResourceProvider class ");
		return retVal;
	}
