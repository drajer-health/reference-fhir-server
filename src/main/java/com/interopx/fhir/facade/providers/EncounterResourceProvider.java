...
/**
 * This class handles request for EncounterResource Resource. It generates a
...
public class EncounterResourceProvider extends AbstractJaxRsResourceProvider<Encounter> {
	 ...
	 * this method: http://<server name>/<context>/fhir/Encounter/1
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public Encounter readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails, @IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with Encounter Read operation :{} at {}",theId.getIdPart(),Instant.now());
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.ENCOUNTER.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.ENCOUNTER.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.ENCOUNTER.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of EncounterResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in EncounterResourceProvider class : ");
		return encounter;
	}
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = Encounter.SP_RES_ID) TokenAndListParam theId,

			@Description(shortDefinition = "Identifier(s) by which this encounter is knownr") @OptionalParam(name = Encounter.SP_IDENTIFIER) TokenAndListParam theIdentifier,

			@Description(shortDefinition = "A date within the period the Encounter lasted") @OptionalParam(name = Encounter.SP_DATE) DateAndListParam theDate,

			@Description(shortDefinition = "Specific type of encounter") @OptionalParam(name = Encounter.SP_TYPE) TokenAndListParam theType,

			@Description(shortDefinition = "The patient or group present at the encounter") @OptionalParam(name = Encounter.SP_PATIENT) ReferenceAndListParam thePatient,

			@Description(shortDefinition = "Classification of patient encounter") @OptionalParam(name = Encounter.SP_CLASS) TokenAndListParam theClass,

			@Description(shortDefinition = "planned | arrived | triaged | in-progress | onleave | finished | cancelled +") @OptionalParam(name = Encounter.SP_STATUS) TokenAndListParam theStatus,

			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with Encounter search with parameters :{} at {}" , request.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all Encounter search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.ENCOUNTER.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() for EncounterResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in EncounterResourceProvider class ");
		return retVal;
	}
