...
/**
 * This class handles request for Patient Resource. It generates a parameter resource based on read/search request types
...
public class PatientResourceProvider extends AbstractJaxRsResourceProvider<Patient> {
	 ...
	 */
	@Read
	public Patient readOrVread(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails, @IdParam IdType theId)
			throws Exception {
logger.debug("Request received on FHIR FACADE with Patient Read operation :{} at {}",theId.getIdPart(),Instant.now());
		Patient thePatient = null;
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(request);
			logger.debug("Using Token to send messages:::::{}",token);
			...
			pathParams.put("resourceType",ResourceTypes.PATIENT.toString());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId, pathParams.get("practiceId"), ResourceTypes.PATIENT.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle, ResourceTypes.PATIENT.toString());
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in readOrVread() of PatientResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in PatientResourceProvider class ");
		return thePatient;
	}
	 ...
	 */
	@Search()
	public Bundle search(HttpServletRequest theRequest, HttpServletResponse theResponse, RequestDetails theRequestDetails,

			@Description(shortDefinition = "The resource identity")
		    @OptionalParam(name = Patient.SP_RES_ID)
		    TokenAndListParam theId,
			@Description(shortDefinition = "A patient identifier") @OptionalParam(name = Patient.SP_IDENTIFIER) TokenAndListParam theIdentifier,
			...
			@Description(shortDefinition = "The patient's date of birth") @OptionalParam(name = Patient.SP_BIRTHDATE) DateAndListParam theBirthdate,
		...
		if (StringUtils.isNotBlank(theRequest.getQueryString())) {
			logger.debug(
					"Request received on FHIR FACADE with Patient search with parameters :{} at {}" , theRequest.getQueryString() ,Instant.now());
		} else {
			logger.debug("Request received for all Patient search");
		}
		...
		try {
			String token = CommonUtil.getAccessTokenFronRequestHeader(theRequest);
			boolean isValid = ResourceProviderValidator.validateRequest(paramMap, ResourceTypes.PATIENT.toString());

			if (!isValid) {
				String paramList = CommonUtil.getRequestedParameters(theRequest);
				throw new InvalidRequestException("400 Bad Request: Unable to process request",
						CommonUtil.getOperationOutcome(paramList));
			}
			String requestId = CommonUtil.generateId().toString();
			Map<String, String> pathParams = CommonUtil.getPathParams(theRequest.getRequestURL());
			pathParams.put("queryParameters", theRequest.getQueryString());
			pathParams.put("requestId",requestId);
			pathParams.put("resourceType",ResourceTypes.PATIENT.toString());
logger.debug("{}{}{} Constructing the Parrameter resource at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
...
logger.debug("{}{}{} Message sent to OutQueue at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					Bundle bundle = (Bundle) fhirContext.newJsonParser().parseResource(responseLog.getPayload());
					if (bundle.hasEntry()) {
						Object obj = ResourceUtil.getResourceFromBundleForSearch(bundle, theRequest);
						if (obj instanceof OperationOutcome) {
							service.updateStatus(requestId, Status.ERROR.toString());
							throw new InvalidRequestException("400 Bad Request: Unable to process request",
									(OperationOutcome) obj);
...
logger.debug("{}{}{} Sending the response at {}",AppConstants.DEBUG_REQUEST_ID,requestId,AppConstants.DEBUG_FHIR_FACADE_EVENT,Instant.now());
			...
			logger.error("Exception in search() of PatientResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in PatientResourceProvider class");
		return retVal;
	}
