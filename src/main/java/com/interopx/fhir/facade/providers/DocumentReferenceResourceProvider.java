...
/**
 * This class handles request for DocumentReference Resource. It generates a
...
public class DocumentReferenceResourceProvider extends AbstractJaxRsResourceProvider<DocumentReference> {
	 ...
	 * this method: http://<server name>/<context>/fhir/DocumentReference/1
	 * 
	 * @param request
	 * @param response
	 ...
	 */
	@Read
	public DocumentReference readOrVread(HttpServletRequest request, HttpServletResponse response,
			RequestDetails theRequestDetails, @IdParam IdType theId) throws Exception {
		logger.debug("Request received on FHIR FACADE with DocumentReference Read operation :{} at {}",
		...
		try {
			...
			pathParams.put("resourceType", ResourceTypes.DOCUMENTREFERENCE.toString());
			...
			RequestResponseLog requestResponseLog = CommonUtil.generateRequestLog(requestId,
					pathParams.get("practiceId"), ResourceTypes.DOCUMENTREFERENCE.toString());
			...
			if (responseLog != null) {
				if (responseLog.getStatus().equals(Status.COMPLETED.toString())) {
					...
					Resource obj = ResourceUtil.getResourceFromBundleForRead(bundle,
							ResourceTypes.DOCUMENTREFERENCE.toString());
			...
			logger.error("Exception in readOrVread() of DocumentReferenceResourceProvider class : ");
			throw e;
		}
		logger.debug("End readOrVread() in DocumentReferenceResourceProvider class ");
	...
	public Bundle search(HttpServletRequest request, HttpServletResponse response, RequestDetails theRequestDetails,
			...
			@Description(shortDefinition = "The resource identity") @OptionalParam(name = DocumentReference.SP_RES_ID) TokenAndListParam theId,
			@Description(shortDefinition = "When this document reference was created") @OptionalParam(name = DocumentReference.SP_DATE) DateRangeParam theDate,

			@Description(shortDefinition = "Kind of document (LOINC if possible)") @OptionalParam(name = DocumentReference.SP_TYPE) TokenAndListParam theType,

			@Description(shortDefinition = "Who/what is the subject of the document") @OptionalParam(name = DocumentReference.SP_PATIENT) ReferenceAndListParam thePatient,

			@Description(shortDefinition = "Encounter DocumentReference was part of") @OptionalParam(name = "encounter") ReferenceAndListParam theEncounter,

			@Description(shortDefinition = "Categorization of document") @OptionalParam(name = DocumentReference.SP_CATEGORY) TokenAndListParam theCategory,

			@Description(shortDefinition = "current | superseded | entered-in-error") @OptionalParam(name = DocumentReference.SP_STATUS) TokenAndListParam theStatus,

			@Description(shortDefinition = "Time of service that is being documented") @OptionalParam(name = DocumentReference.SP_PERIOD) DateRangeParam thePeriod,
			@IncludeParam(allow = { "DocumentReference:author",
					"DocumentReference:custodian" }) Set<Include> theIncludes,
			@IncludeParam(reverse = true, allow = { "Provenance:target" }) Set<Include> theRevIncludes)
			throws Exception {
		if (StringUtils.isNotBlank(request.getQueryString())) {
			logger.debug("Request received on FHIR FACADE with DocumentReference search with parameters :{} at {}",
					request.getQueryString(), Instant.now());
		} else {
			logger.debug("Request received for all DocumentReference search");
		}
		...
		try {
			...
			logger.error("Exception in search() for DocumentReferenceResourceProvider class : ");
			throw e;
		}
		logger.debug("End search() in DocumentReferenceResourceProvider class ");
		return retVal;
	}
